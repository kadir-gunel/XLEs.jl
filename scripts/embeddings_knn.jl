...
file = "../data/embeddings/en"
voc, X = readBinaryEmbeddings(file);
X = Matrix(PermutedDimsArray(X, (2, 1));
X20k = X[:, 1:Int(20e3)];
...
function parallelIdx(R::Matrix)
    top_idx = Matrix{Int64}(undef, size(result))
    @threads for i in axes(R, 2)
            top_idx[:, i] = sortperm(R[:, i])
        end
    return top_idx
end
...
@time result = parallelCosine(X20k, X20k);
@time top_idx = parallelIdx(result);
