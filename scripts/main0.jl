...
for trg in ["es", "de",  "fi", "it"]
    ...
    for i in 1:1 # each language mapping is repeated 10 times
        ...
        src_idx, trg_idx = buildSeedDictionary(@view(X[:, 1+Int(40e3):Int(60e3)]), @view(Y[:, 1+Int(60e3):Int(80e3)]))
        ...
        while true
            ...
            src_idx, trg_idx, objective, W = train(@view(X[:, 1+Int(40e3):Int(60e3)]), @view(Y[:, 1+Int(60e3):Int(80e3)]), Wt_1, src_idx, trg_idx, src_size, trg_size, keep_prob, objective; stop=stop, time=false, lambda=λ)
